<template>
  <div>
    <button @click="exportExcel" class="button is-small is-primary">
      <span class="icon">
        <i class="fas fa-file-download"></i>
      </span>
      <span>Xuất File VNPT</span>
    </button>
  </div>
</template>
<script>
  ...
  methods: {
    ...
    async exportExcel() {
      ...
      this.data_execl.forEach((item, index) => {
        ...
        row.getCell(6).value = item.maphuongan; // Cột F

        row.getCell(11).value = item.tienluongcs; // Cột K
        ...
        row.getCell(24).value = item.sothang; // Cột X
        row.getCell(25).value = `${item.madaily} - ${item.tennguoitao}`; // Cột Y
